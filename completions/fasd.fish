...
complete -c fasd -s e -a "subl atom" -d "Set command to execute on the result file" -f
complete -c fasd -s b -a "spotlight viminfo recently-used current" -d "Only use given backend" -f
complete -c fasd -s B -a "spotlight viminfo recently-used current" -d "Add additional backend" -f
